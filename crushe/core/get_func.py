        ...
        try:
            ...
            if msg.media == MessageMediaType.VIDEO and msg.video.mime_type in ["video/mp4", "video/x-matroska"]:
                ...
                if duration <= 300:
                    ...
                    if msg.pinned_message:
                        try:
                            await crushe_msg.pin(both_sides=True)
                        except Exception:
                            await crushe_msg.pin()
                    try:
                        await sent_doc.copy(LOG_GROUP)
                    except Exception as e:
                ...
                try:
                    crushe_msg = await app.send_video(
                        chat_id=target_chat_id,
                        video=file,
                        caption=caption,
                ...
                crushein = await app.send_photo(chat_id=target_chat_ids, photo=file, caption=caption)
                if msg.pinned_message:
                    try:
                        await sent_doc.pin(both_sides=True)
                    except Exception:
                ...
                try:
                    if msg.media == MessageMediaType.DOCUMENT:
                        crushe_msg = await app.send_document(
                            chat_id=target_chat_id,
                            document=file,
                            caption=caption,
                            thumb=thumb_path,
